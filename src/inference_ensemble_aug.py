...
INFERENCE_TO_TRAIN_FORMAT = True
...
PATH_AUG_OUTPUT = "notebooks/articles_dev_aug.json"
...
if __name__ == "__main__":
    ...
    if INFERENCE_TO_TRAIN_FORMAT:
        with open(PATH_AUG_ARTICLES, "r", encoding="utf-8") as f:
            data = json.load(f)
        df_infer = prepare_json_to_df(PATH_AUG_ARTICLES, False, 'augment')
    else:
    ...
    results_ = {} # Dict to store final JSON output {pmid: {"entities": [...]}}
    ...
    with torch.no_grad():
        for batch in tqdm(inference_dataloader_title, desc="Inference"):
            ...
            for i in range(len(batch['input_ids'])):
                ...
                results_[doc_ids[i]] = {"entities": result} # Store results in the dictionary
        ...
        for batch in tqdm(inference_dataloader_abstract, desc="Inference"):
            ...
            for i in range(len(batch['input_ids'])):
                ...
                if doc_ids[i] in results_:
                    results_[doc_ids[i]]["entities"].extend(result)
                else:
                    results_[doc_ids[i]] = {"entities": result}
        
        if INFERENCE_TO_TRAIN_FORMAT:
            for doc_id in results_.keys():
                results_[doc_id]['metadata'] = {
